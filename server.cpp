...
void* handle_recv (void* arguments) {
	...
	while(true) {
        ...
        if (strcmp(data, "close") == 0 || strlen(data) == 0){
			close(client_sockfd);
			printf("[LOG] %s:%d > client %s left\n", inet_ntoa(clientaddr.sin_addr), clientaddr.sin_port, cli_index.c_str());
			break;
		}
        ...
        printf("[%s->S] %s:%d > %s\n", cli_index.c_str(), inet_ntoa(clientaddr.sin_addr), clientaddr.sin_port, data);
...
int main() {
    ...
    while(true) {
        ...
        printf("Client %d with IP: %s and PORT: %d has connected\n", client_count + 1, inet_ntoa(clientaddr.sin_addr), clientaddr.sin_port);
